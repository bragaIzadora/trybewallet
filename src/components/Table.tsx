...
import { GlobalState, Expenses } from '../types';
import { Delete } from '../redux/actions';

function Table() {
  const { expenses }: { expenses: Expenses[] } = useSelector((
    state: GlobalState,
  ) => state.wallet);
  ...
  return (
    <div>
      <table>
        ...
        <tbody>
          {
            expenses.length > 0 && expenses.map((expense:Expenses) => {
              return (
                <tr key={ expense.id }>
